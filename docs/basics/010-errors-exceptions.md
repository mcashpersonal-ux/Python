...
the except block runs instead of crashing. Catch the narrowest
type you can - ValueError, not Exception.
...
the happy path dies. Prefer with for this (see file-handling.md
finish). But finally exists for cases with cannot express.
...
enforce contracts on your functions.
...
cause. Exception groups in tracebacks, perfect for debugging.
...
It avoids race conditions and duplicated lookups.
