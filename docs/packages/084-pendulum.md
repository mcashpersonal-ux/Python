...
Pendulum is most useful when its output is part of a repeatable project workflow rather than a one-off local command. Start with the smallest command below, then put the same check into the project task runner or continuous-integration job.
...
Store absolute instants in UTC at system boundaries. Treat local civil times and recurring schedules separately because daylight-saving rules can change elapsed time.
...
[Pendulum documentation](https://pendulum.eustacegu.github.io/)
